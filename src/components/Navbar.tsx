    ...
    <nav className="fixed w-full top-0 z-50 bg-gradient-to-b from-slate-900 to-transparent backdrop-blur-md border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-20">
          {/* Layout a 3 colonne per centrare perfettamente */}
          <div className="flex items-center justify-between w-full">
            {/* Colonna sinistra: Logo */}
            <div className="flex-1 flex justify-start">
              <Link to="/" className="flex items-center gap-2">
                <img src="/apple-touch-icon.png" alt="Giuseppe Russo" width={60} height={60} />
              </Link>
            </div>

            {/* Colonna centro: Menu (centrato perfettamente) */}
                ...
                {menuItems.map((item) => {
                  ...
                  return isHashLink ? (
                    <a
                      ...
                      className="text-gray-300 hover:text-white transition-colors duration-300 font-medium cursor-pointer whitespace-nowrap"
                    >
                      {item.label}
                    </a>
                  ) : (
